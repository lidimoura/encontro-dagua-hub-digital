...
export const BoardStrategyHeader: React.FC<BoardStrategyHeaderProps> = ({ board }) => {
  ...
  const calculatedProgress = React.useMemo(() => {
    ...
    if (type === 'percentage') {
      ...
      const won = boardDeals.filter(d => d.status === 'CLOSED_WON').length;
              ...
              <div className="md:col-span-3 flex flex-col justify-center px-4 border-r border-slate-100 dark:border-white/5 relative">
                <div className="flex items-center justify-between mb-1">
                  ...
                  {board.agentPersona && (
                    <button
                      onClick={() => {
                        // Dispatch custom event to open FloatingAIWidget
                        window.dispatchEvent(new CustomEvent('openAIWidget', {
                          detail: { boardId: board.id, agentName: board.agentPersona?.name }
                        }));
                      }}
