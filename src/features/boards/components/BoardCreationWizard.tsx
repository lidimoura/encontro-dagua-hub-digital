...
import { X, Sparkles, Loader2, Send, MessageSquare, LayoutTemplate } from 'lucide-react';
...
interface BoardCreationWizardProps {
  isOpen: boolean;
  onClose: () => void;
  onCreate: (board: Omit<Board, 'id' | 'createdAt'>) => void;
...
export const BoardCreationWizard: React.FC<BoardCreationWizardProps> = ({
  ...
  const handleInstallJourney = async (templatePath: string) => {
    ...
    try {
      ...
      // Install all boards in the journey
      for (const boardDef of journey.boards) {
  ...
  const handleInstallOfficialJourney = (journeyId: string) => {
    ...
    try {
      ...
      setChatMessages(prev => [
        ...prev,
        {
          ...
          proposalData: hasChanges ? response.board : undefined,
        },
      ]);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        className={`relative z-10 w-full ${isChatMode ? 'max-w-7xl' : 'max-w-6xl'} bg-white dark:bg-dark-card rounded-2xl shadow-2xl overflow-y-auto flex flex-col max-h-[90vh] transition-all duration-300`}
      >
        {/* Header */}
