        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={async () => {
              ...
              if (count > 0) {
                addToast(`${count} ${t('alerts').toLowerCase()}!`, 'warning');
              } else {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            <h3 className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-2">
              {t('walletDistribution')}
            </h3>
            <div className="flex items-end gap-2 mb-2">
              <span className="text-2xl font-bold text-slate-900 dark:text-white">
                {activePercent}%
              </span>
              <span className="text-xs text-green-500 font-bold mb-1">{t('active')}</span>
            </div>
            {/* Progress bar */}
            ...
            <div className="flex justify-between mt-2 text-xs text-slate-500">
              <div className="flex items-center gap-1">
                <div className="w-2 h-2 rounded-full bg-green-500"></div> {t('active')} ({activeContacts.length})
              </div>
              <div className="flex items-center gap-1">
                <div className="w-2 h-2 rounded-full bg-yellow-500"></div> {t('inactive')} ({inactiveContacts.length})
              </div>
              <div className="flex items-center gap-1">
                <div className="w-2 h-2 rounded-full bg-red-500"></div> {t('churn')} ({churnedContacts.length})
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div className="flex items-end gap-2">
              <span className="text-2xl font-bold text-slate-900 dark:text-white">
                {riskyCount} {t('clients')}
              </span>
              <span className="text-xs text-red-500 font-bold mb-1">{t('alerts')}</span>
            </div>
          ...
          <div className="glass p-6 rounded-xl border border-slate-200 dark:border-white/5 shadow-sm">
            ...
            <div className="flex items-end gap-2">
              ...
              <span className="text-xs text-green-500 font-bold mb-1">{t('avgDays')}</span>
            </div>
            <p className="text-xs text-slate-500 mt-2">{t('ltvDesc')}</p>
          </div>
        </div>
      </div>
