...
export const useDashboardMetrics = () => {
    ...
    const wonDeals = deals.filter(l => l.status === DealStatus.CLOSED_WON);
    const winRate = deals.length > 0 ? (wonDeals.length / deals.length) * 100 : 0;
    const pipelineValue = deals
        .filter(l => l.status !== DealStatus.CLOSED_LOST && l.status !== DealStatus.CLOSED_WON)
    ...
    // Sales Cycle Metrics
    const closedDeals = deals.filter(d => d.status === DealStatus.CLOSED_WON || d.status === DealStatus.CLOSED_LOST);
