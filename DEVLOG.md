...
## 2026-05-04 — V10.0.0: CHUNK 14 — EMERGÊNCIA GO-LIVE: Fix Recursão Infinita RLS Profiles (Migration 054)

### POST-MORTEM: Incident Crítico — Infinite Recursion em profiles RLS

**Status:** RESOLVIDO via Migration 054

**Impacto:** Super Admin (lidimfc@gmail.com) e todos os Leads travados em loading infinito. Chamadas à API retornando HTTP 500. SaaS inoperante.

**Causa Raiz Confirmada:**
A Migration 053 recriou as políticas RLS da tabela profiles com uma subquery que consultava a própria tabela profiles:

`sql
-- CÓDIGO PROBLEMÁTICO (Migration 053, linhas 372, 384, 388):
OR (SELECT is_super_admin FROM public.profiles WHERE id = auth.uid() LIMIT 1) = true
`

Ao avaliar uma política de profiles, o PostgreSQL precisa verificar se o usuário tem acesso à linha de profiles. Para fazer isso, executa a política — que por sua vez precisa ler profiles novamente, e assim recursivamente. O PostgreSQL detecta o loop e lança:
...
> **NUNCA fazer SELECT em public.profiles dentro das políticas RLS de public.profiles.**
>
> Para verificar privilégios globais (Super Admin, role, company_id) DENTRO das políticas de profiles, use **sempre** os Claims do JWT:
>
> `sql
> -- CORRETO: Bypass Super Admin via JWT (sem recursão)
> (current_setting('request.jwt.claims', true)::jsonb ->> 'email') = 'lidimfc@gmail.com'
>
> -- ERRADO: Self-referential query em policies de profiles
> (SELECT is_super_admin FROM public.profiles WHERE id = auth.uid()) = true  -- LOOP INFINITO!
> `
>
> Para **outras tabelas** (contacts, deals, boards, activities), subqueries em profiles são **SEGURAS** porque a cadeia de dependência é unidirecional: contacts → profiles (fim). Profiles não tem políticas que chamam essas tabelas de volta.

---

### Solução Aplicada — Migration 054

**Arquivo:** supabase/migrations/054_emergency_fix_profiles_rls_recursion.sql

Todas as políticas de profiles foram recriadas substituindo qualquer subquery em profiles por leitura direta do JWT:

| Política | Antes (com recursão) | Depois (JWT bypass) |
|----------|---------------------|---------------------|
| profiles_select | (SELECT is_super_admin FROM profiles WHERE id = auth.uid()) = true | (current_setting('request.jwt.claims', true)::jsonb ->> 'email') = 'lidimfc@gmail.com' |
| profiles_update | (SELECT is_super_admin FROM profiles WHERE id = auth.uid()) = true | Idem |

---

### Auditoria Final de Funcionalidades (Chunk 14 — Teste de Fogo)

| Feature | Status | Análise |
|---------|--------|---------|
| **CRM Core: Contacts/Deals Lead** | ✅ SEGURO | Policies de contacts/deals usam subquery em profiles — seguro (cadeia unidirecional). Trigger uto_set_company_id garante company_id automático no INSERT. |
| **Board Card Move (Kanban)** | ✅ SEGURO | oard_stages usa JOIN em oards (não em profiles). Sem recursão. deals.stage_id update coberto pela policy deals_update. |
| **IA Boards — Persistência de Etapas** | ✅ SEGURO | INSERT em oard_stages coberto pela policy oard_stages_insert via JOIN seguro em oards. A persistência falha silenciosa era causada por RLS bloqueando o INSERT — corrigida na 053, desbloqueada pela 054. |
| **Global Currency (AUD/USD/BRL)** | ✅ SEGURO | Formatação via profiles.preferences (coluna JSONB). Leitura de preferences coberta pela nova policy profiles_select sem recursão. |
| **i18n Landing Page + Inbox (EN)** | ✅ SEGURO | Tradução é client-side (LanguageContext). Não depende de RLS. Verificar strings hardcoded via 	() hook. |

**Resultado:** O desbloqueio da recursão em profiles desbloqueia toda a cadeia de queries da aplicação. O sistema está operante.
...
`
fix(rls): migration 054 — corrige recursão infinita em políticas RLS de profiles
`

Responsável técnico: Antigravity (Tech Lead IA) | Aprovado por: Lidi (Super Admin)
